...
CAPACITY_ENDPOINT = 'http://capacity.example.com/v1'
...
def _get_normalized_token_data(**kwargs):
    ...
    ref['capacity_url'] = ref.get('capacity_url', CAPACITY_ENDPOINT)
...
def generate_v2_project_scoped_token(**kwargs):
    ...
    # we only care about Capacity and Keystone endpoints
    o['access']['serviceCatalog'] = [
        {'endpoints': [
            {'publicURL': ref.get('capacity_url'),
             'id': uuid.uuid4().hex,
             'region': 'RegionOne'
             }],
         'endpoints_links': [],
